...
def test_urls_availability(api_client, five_ingredients):
...
@pytest.mark.django_db
def test_urls_unauthenticated_permissions(api_client, five_ingredients):
    response = api_client.get(INGREDIENT_URL)
...
@pytest.mark.django_db
def test_ingredients_have_no_pagination(api_client, five_ingredients):
    response = api_client.get(INGREDIENT_URL)
...
@pytest.mark.django_db
def test_api_response_fields(api_client, five_ingredients):
...
@pytest.mark.django_db
def test_ingredient_search_with_full_keyword(api_client, five_ingredients):
    search_param = 'search'
...
@pytest.mark.django_db
def test_ingredient_search_with_partial_keyword(api_client, five_ingredients):
    data = {'name': 'po'}
    response = api_client.get(INGREDIENT_URL, data=data)
    assert len(response.data) == 1, (
